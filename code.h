...
public:
	...
	static const array<tuple<int32_t,QString, QString, QString>, 255> INSTRUCTIONS;
...
struct InstructionContext {
	...
	QString formatter;
	bool wide = false;
	int32_t parameter1 = 0x80000000;   //用一个大的容器存储参数
	int32_t parameter2 = 0x80000000;
	void* extra = nullptr;
	QString remark;
};
...
class CodeVisitor {
public:
	virtual void visit(const InstructionContext& context) = 0;
};
...
class CodeAttribute {
public:
	CodeAttribute(const AttributeInfo* c, const ConstantPool* cp);
	const AttributeInfo* getCode();

	uint16_t getMaxStack() const;
	uint16_t getMaxLocals() const;
	uint32_t getCodeLength() const;
	uint16_t getExceptionTableLength() const;
	const vector<shared_ptr<AttributeInfo>>& getAttributes() const;

	void each(CodeVisitor& visitor);

private:
	const AttributeInfo* code;
	uint16_t maxStack;
	uint16_t maxlocals;
	uint32_t codeLength;
	uint32_t codeStart;    //相对于codeBody处的
